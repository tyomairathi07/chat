...
roomRef.on('child_added', function(childSnapshot, prevChildKey) {
	$('#log').append('child_added: ' + childSnapshot.key + '<br>');
})

roomRef.on('child_removed', function(oldChildSnapshot) {
	var row = oldChildSnapshot.child('row-index').val();
	var cell = oldChildSnapshot.child('cell-index').val();

	$('#log').append('child_removed: ' + oldChildSnapshot.key + ' ' + row + ',' + cell + '<br>');
})
